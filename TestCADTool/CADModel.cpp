...
{
	auto iter = mapBaseElemType2Str.find(CADModelTool::BaseElemType::BasePoint);
	if (iter == mapBaseElemType2Str.cend())
		return 1;
...
{
	auto iter = mapBaseElemType2Str.find(CADModelTool::BaseElemType::BaseVector);
	if (iter == mapBaseElemType2Str.cend())
		return 1;
	elemNode->SetAttribute("type", iter->second.c_str());
	tinyxml2::XMLElement* pNode = elemNode->InsertNewChildElement("x");
...
{	
	//约束类型
	auto iterStrType = mapConsType2Str.find(cons.type);
	if (iterStrType == mapConsType2Str.cend())
		return 1;
	...
	auto iterStrType = mapElemType2Str.find(cons.pElem1->type);
	if (iterStrType == mapElemType2Str.cend())
		return 1;
	...
	auto iterStrType = mapElemType2Str.find(cons.pElem2->type);
	if (iterStrType == mapElemType2Str.cend())
		return 1;
...
int Loc_GetDoubleValue(const tinyxml2::XMLElement* pMatrixNode,std::string name,double &value)
{
...
int Loc_GetPoint(const tinyxml2::XMLElement* pNode, CADModelTool::CADPoint& point)
{
...
int Loc_GetBasePointElem(const tinyxml2::XMLElement* pBaseElem,std::pair<std::string, CADModelTool::CADPoint>& pairPnt)
{
	...
	auto baseElemType = mapStr2BaseElemType.find(elemTypeStr);
	if (baseElemType == mapStr2BaseElemType.cend())
		return 1;
...
int Loc_SetPlaneData(const std::pair<std::string, CADModelTool::CADPoint>& pairPnt, CADModelTool::CADPlane& plane)
{
...
int Loc_GetPlaneElem(const std::string& strAxiliary,const tinyxml2::XMLElement* pReConsElemNode, CADModelTool::PlaneElem** ppPlaneElem)
{
...
int Loc_GetConsElem(const tinyxml2::XMLElement* pConsCompNode, CADModelTool::BaseAsmElem** ppBaseElem)
{
	/*1 auxiliary_file*/
	...
	if (!consElemTypeStr)
		return 1;
	auto consElemTypeIter = mapStr2ElemType.find(consElemTypeStr);
	if (consElemTypeIter == mapStr2ElemType.cend())
		return 1;
...
int Loc_GetConstrant(const tinyxml2::XMLElement* pConsNode, CADModelTool::CADConstraint& constraint)
{
	/*1 type*/
	...
	auto consTypeIter = mapStr2ConsType.find(consTypeStr);
	if (consTypeIter == mapStr2ConsType.cend())
		return 1;
...
int Loc_GetConstrantsGroup(const tinyxml2::XMLElement* pConsGroupNode, std::vector<CADModelTool::CADConstraint> &vecConstraint)
{
...
int Loc_GetMatrix(const tinyxml2::XMLElement* pMatrixNode, CADModelTool::CADMatrix& matrix)
{ 
	double x1;
	...
	if (Loc_GetDoubleValue(pMatrixNode, "z3", z3))
		return 1;
	matrix.x = CADModelTool::CADVector{ x1,x2,x3 };
	matrix.y = CADModelTool::CADVector{ y1,y2,y3 };
	matrix.z = CADModelTool::CADVector{ z1,z2,z3 };
	return 0;
}


int Loc_GetPartInfo(const tinyxml2::XMLElement* pPartNode, CADModelTool::CADPartInfo& partInfo)
{
...
int Loc_GetSubNode(CADModelTool::CADComponent** ppParentNode,tinyxml2::XMLElement* pCurElem)
{
	/*get name*/
...
//int Loc_Traversal(CADModelTool::CADComponent* pParentNode,tinyxml2::XMLNode *pNode)
//{
//	if (!pNode)
//		return 0;
//	auto pCurElem = pNode->FirstChildElement("sub_node");
//	if (Loc_GetSubNode(pParentNode, pCurElem))
//		return 1;
//
//	while (pCurElem)
//	{
//		if (Loc_Traversal(pParentNode, pCurElem))
//			return 1;
//		pCurElem = pCurElem->NextSiblingElement();
//	}
//	return 0;
//}
//
//int Loc_Traversal(tinyxml2::XMLNode* pNode)
//{
//	if (!pNode)
//		return 0;
//	auto pCurElem = pNode->FirstChildElement("sub_node");
